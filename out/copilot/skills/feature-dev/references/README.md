...
The Feature Development Skill provides a systematic 7-phase approach to building new features. Instead of jumping straight into code, it guides you through understanding the codebase, asking clarifying questions, designing architecture, and ensuring quality—resulting in better-designed features that integrate seamlessly with your existing code.
...
This skill embeds these practices into a structured workflow that runs automatically when you invoke `feature-dev`.
...
**What happens:**
- Launches 2-3 `code-explorer` agents in parallel using the `agent` tool
- Each agent explores different aspects (similar features, architecture, UI patterns)
- Agents return comprehensive analyses with key files to read
- Reads all identified files using the `read` tool to build deep understanding
...
**What happens:**
- Launches 2-3 `code-architect` agents with different focuses using the `agent` tool:
...
- Reads all relevant files identified in previous phases using the `read` tool
- Implements following chosen architecture using the `edit` tool
- Follows codebase conventions strictly
- Writes clean, well-documented code
- Updates `todo` list as progress is made
...
**What happens:**
- Launches 3 `code-reviewer` agents in parallel using the `agent` tool with different focuses:
